...
def get_dict_similarity(a, b, w=None):
    ...
    for key in b:
        ...
        if key not in a:
            mismatch += 0.5 + weight(b[key],
                                     type(b[key]))  # Add 0.5 because missing key is worse than mismatching value
...
def find_alert_matching(alerts_a, alerts_b, early_stopping_threshold=0.0, w=None, min_alert_match_similarity=0.0):
    ...
    similarities = []
    alert_pairs = []
    max_allowed_total_dist = (1 - early_stopping_threshold) * max(len(alerts_a),
                                                                  len(alerts_b))  # When using early stopping, this must correspond to criteria for declaring groups a match or not
    current_max_dist = abs(len(alerts_a) - len(
        alerts_b))  # If number of alerts are not equal, the alerts that will not find a match are counted in as contributing to the distance as perfect mismatches
    for a in alerts_a:
        ...
        if current_max_dist > max_allowed_total_dist and early_stopping_threshold != 0.0:  # Never stop when early_stopping_threshold is 0.0
            ...
            return {}
    return {y: x for x, y in sorted(zip(similarities, alert_pairs), key=lambda pair: pair[0],
                                    reverse=True)}  # key to avoid that sort takes second list into account in case first list has duplicates


def get_group_similarity_avg(group_a, group_b, early_stopping_threshold=0.0, w=None, min_alert_match_similarity=0.0,
                             alignment_weight=0.0, partial=False):
...
def get_group_similarity_exact(group_a, group_b, early_stopping_threshold=0.0, w=None, min_alert_match_similarity=0.0,
                               alignment_weight=0.0, partial=False):
    ...
    if min(len(group_a.alerts), len(group_b.alerts)) / max(len(group_a.alerts),
                                                           len(group_b.alerts)) < early_stopping_threshold:
        ...
        return 0.0
    s = 0.0
    alert_matching = find_alert_matching(group_a.alerts, group_b.alerts,
    ...
    if partial == False:
        ...
        if alignment_weight != 0.0 and len(alignment_a) > 0 and len(alignment_b) > 0:
            ...
            s = s * (1 - alignment_weight) + (editdistance.eval(alignment_a, alignment_b) / max(len(alignment_a),
                                                                                                len(alignment_b))) * alignment_weight
    else:
        s /= len(group_a.alerts)  # Only care about how well group_a is represented in group_b
    return s


def get_group_similarity_bag(group_a, group_b, early_stopping_threshold=0.0, w=None, min_alert_match_similarity=0.0,
                             alignment_weight=0.0, partial=False):
    s = 0.0
    ...
    if min(len(group_a.bag_of_alerts), len(group_b.bag_of_alerts)) / max(len(group_a.bag_of_alerts),
                                                                         len(group_b.bag_of_alerts)) < early_stopping_threshold:
        return 0.0
    alert_matching = find_alert_matching(group_a.bag_of_alerts.keys(), group_b.bag_of_alerts.keys(),
                                         early_stopping_threshold=0.0, w=w,
                                         min_alert_match_similarity=min_alert_match_similarity)  # Set early stopping to 0.0 for bag since grouping criteria do not match
    used_a = []
    used_b = []
    for a, b in alert_matching:
        if a not in used_a and b not in used_b:
            ...
            if group_a_type is tuple and group_b_type is tuple:
                ...
                if min(group_a.bag_of_alerts[a][1], group_b.bag_of_alerts[b][1]) >= max(group_a.bag_of_alerts[a][0],
                                                                                        group_b.bag_of_alerts[b][0]):
                    s += 1
                ...
                s += min(group_a.bag_of_alerts[a], group_b.bag_of_alerts[b]) / max(group_a.bag_of_alerts[a],
                                                                                   group_b.bag_of_alerts[b])
    if partial == False:
...
def get_group_similarity_alignment(group_a, group_b, early_stopping_threshold=0.0, w=None,
                                   min_alert_match_similarity=0.0, alignment_weight=0.0, partial=False):
    if min(len(group_a.merge_seq), len(group_b.merge_seq)) / max(len(group_a.merge_seq),
                                                                 len(group_b.merge_seq)) < early_stopping_threshold:
        return 0.0
    s = 0.0
    alert_matching = find_alert_matching(group_a.bag_of_alerts.keys(), group_b.bag_of_alerts.keys(),
                                         early_stopping_threshold=0.0, w=w,
...
def get_group_similarity(group_a, group_b, early_stopping_threshold=0.0, w=None, min_alert_match_similarity=0.0,
                         alignment_weight=0.0, strategy='best', partial=False):
    # alerts_a, alerts_b are lists of alert objects
    if strategy == 'avg':
        return get_group_similarity_avg(group_a, group_b, early_stopping_threshold, w, min_alert_match_similarity,
                                        alignment_weight, partial)
    elif strategy == 'best':
        return get_group_similarity_exact(group_a, group_b, early_stopping_threshold, w, min_alert_match_similarity,
                                          alignment_weight, partial)
    elif strategy == 'bag':
        s_bag = 0
        if alignment_weight != 1.0:
            s_bag = get_group_similarity_bag(group_a, group_b, early_stopping_threshold, w, min_alert_match_similarity,
                                             alignment_weight, partial)
        s_alignment = 0
        if alignment_weight != 0.0:
            s_alignment = get_group_similarity_alignment(group_a, group_b, early_stopping_threshold, w,
                                                         min_alert_match_similarity, alignment_weight, partial)
        return s_bag * (1 - alignment_weight) + s_alignment * alignment_weight
    elif strategy == 'ot':
        from similarity.optimal_transport import get_group_similarity_ot
        s = get_group_similarity_ot(group_a, group_b,
                                     early_stopping_threshold=early_stopping_threshold,
                                     w=w,
                                     min_alert_match_similarity=min_alert_match_similarity,
                                     partial=partial)
        if s is None:
            return get_group_similarity_bag(group_a, group_b, early_stopping_threshold, w,
                                            min_alert_match_similarity, alignment_weight, partial)
        return s
    else:
