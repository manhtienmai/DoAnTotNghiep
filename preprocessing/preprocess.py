...
def read_ossec_minimal_json(filename):
  ...
  with open(filename) as f:
    for line in f:
      ...
      alert_obj.timestamp = alert['timestamp'] # them timestamp vào alert
  ...
  with open(filename) as f:
    for line in f:
      ...
      if 'timestamp' in alert_obj.d:
        ts = parser.parse(alert_obj.d['timestamp']).timestamp()
      else:
        ts = parser.isoparse(alert_obj.d['@timestamp']).timestamp()
...
def read_aminer_json(filename):
  ...
  for json_alert in json_alerts:
    ...
    alert_obj.file = filename
    if retry_lines:
      ts = json_alert['LogData']['DetectionTimestamp'][0]
    else:
      ts = json_alert['LogData']['Timestamps'][0]
    alert_obj.timestamp = ts
    alerts.append(alert_obj)
    timestamps.append(ts)
  return alerts, timestamps
