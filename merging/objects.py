    ...
    if delta in self.meta_alerts:
      for meta_alert in self.meta_alerts[delta]:
        if len(group.alerts) * len(meta_alert.alert_group.alerts) > bag_limit:
          ...
          s = similarity.get_group_similarity(group, meta_alert.alert_group, early_stopping_threshold=threshold, w=w, min_alert_match_similarity=min_alert_match_similarity, alignment_weight=alignment_weight, strategy=aggregate_config.group_sim_strategy, partial=partial)
