...
class Mergelist:
    ...
    def __repr__(self):
        ...
        for element in self.elements:
            res += '"' + str(element).replace("\"", "\\\"").replace("'", "\\\"").replace("\n", "\\n") + '", '
...
class MetaAlert:
    ...
    def update_alert(self, match_similarity, min_alert_match_similarity=0.0, max_val_limit=None, min_key_occurrence=0.0,
                     min_val_occurrence=0.0):
        if match_similarity != 1.0:
            ...
            self.alert_group = merge.merge_group(self.mam.kb.get_groups(self),
                                                 min_alert_match_similarity=min_alert_match_similarity,
                                                 max_val_limit=max_val_limit, min_key_occurrence=min_key_occurrence,
    ...
    def __repr__(self):
        res = 'Meta-Alert with ' + str(len(self.mam.kb.get_groups(self))) + ' allocated groups: ' + str(
...
class MetaAlertManager:
    ...
    def add_to_meta_alerts(self, group, delta, threshold, min_alert_match_similarity=0.0, max_val_limit=None,
                           min_key_occurrence=0.0, min_val_occurrence=0.0, bag_limit=2000, w=None, alignment_weight=0.0,
                           force_label=None, partial=False, pair_strategy='best'):
        # Side-effect of this method is to set group.meta_alert
        if len(group.bag_of_alerts) == 0:
            group.create_bag_of_alerts(min_alert_match_similarity, max_val_limit, min_key_occurrence,
                                       min_val_occurrence)
        best_matching_meta_alert, maxSimilarity = self.get_most_similar_meta_alert(group=group, delta=delta,
                                                                                   threshold=threshold,
                                                                                   max_val_limit=max_val_limit,
                                                                                   min_key_occurrence=min_key_occurrence,
                                                                                   min_val_occurrence=min_val_occurrence,
                                                                                   bag_limit=bag_limit, w=w,
        ...
        if maxSimilarity >= threshold:
            ...
            best_matching_meta_alert.update_alert(maxSimilarity, min_alert_match_similarity=min_alert_match_similarity,
                                                  max_val_limit=max_val_limit, min_key_occurrence=min_key_occurrence,
                                                  min_val_occurrence=min_val_occurrence)
            ...
            meta_alert.update_alert(maxSimilarity, min_alert_match_similarity=min_alert_match_similarity,
                                    max_val_limit=max_val_limit, min_key_occurrence=min_key_occurrence,
                                    min_val_occurrence=min_val_occurrence)
    ...
    def get_most_similar_meta_alert(self, group, delta, threshold, min_alert_match_similarity=0.0, max_val_limit=None,
                                    min_key_occurrence=0.0, min_val_occurrence=0.0, bag_limit=2000, w=None,
                                    alignment_weight=0.0, force_label=None, partial=False, pair_strategy='best'):
        if len(group.bag_of_alerts) == 0:
            group.create_bag_of_alerts(min_alert_match_similarity, max_val_limit, min_key_occurrence,
                                       min_val_occurrence)
        maxSimilarity = -1.0
        ...
        if delta in self.meta_alerts:
            for meta_alert in self.meta_alerts[delta]:
                if len(group.alerts) * len(meta_alert.alert_group.alerts) > bag_limit:
                    s = similarity.get_group_similarity(group, meta_alert.alert_group,
                                                        early_stopping_threshold=threshold, w=w,
                                                        min_alert_match_similarity=min_alert_match_similarity,
                                                        alignment_weight=alignment_weight, strategy='bag',
                                                        partial=partial)
                else:
                    s = similarity.get_group_similarity(group, meta_alert.alert_group,
                                                        early_stopping_threshold=threshold, w=w,
                                                        min_alert_match_similarity=min_alert_match_similarity,
                                                        alignment_weight=alignment_weight, strategy=pair_strategy,
                                                        partial=partial)
                if s > maxSimilarity and (
                        force_label is None or meta_alert.alert_group.attacks == force_label):  # If force label is True, only allow meta_alerts with that group label
                    maxSimilarity = s
    ...
    def __repr__(self):
        res = 'Meta-Alert-Manager with ' + str(
            sum([len(groups) for delta, groups in self.meta_alerts.items()])) + ' meta-alerts:'
        i = 1
        for delta, meta_alerts in self.meta_alerts.items():
            for meta_alert in meta_alerts:
                res += '\n' + 'Meta-Alert #' + str(i) + '\n' + str(meta_alert)
...
class KnowledgeBase:
        ...
        if self.limit is None:
            ...
            if meta_alert in self.meta_alert_dict:
                if len(self.meta_alert_dict[meta_alert]) < self.limit:
                    ...
                    self.meta_alert_dict[meta_alert] = self.meta_alert_dict[meta_alert][:self.limit - move_pos - 1] + \
                                                       self.meta_alert_dict[meta_alert][self.limit - move_pos:] + [
                                                           group]
            else:
                self.meta_alert_dict[meta_alert] = [group]
    ...
    def hierarchical_clustering(self, labels, max_val_limit=None, min_key_occurrence=0.0, min_val_occurrence=0.0,
                                early_stopping_threshold=0.0, w=None, alignment_weight=0.0,
                                min_alert_match_similarity=0.0, bag_limit=2000):
        ...
        for delta, groups in self.delta_dict.items():
            # Start the recursion
            ret_str += self.hierarchical_clustering_rec(groups, sim_matrix, {}, labels, max_val_limit=max_val_limit,
                                                        min_key_occurrence=min_key_occurrence,
                                                        min_val_occurrence=min_val_occurrence,
                                                        early_stopping_threshold=early_stopping_threshold, w=w,
                                                        alignment_weight=alignment_weight,
                                                        min_alert_match_similarity=min_alert_match_similarity,
                                                        bag_limit=bag_limit)
    ...
    def hierarchical_clustering_rec(self, groups, sim_matrix, merge_similarities, labels, max_val_limit=None,
                                    min_key_occurrence=0.0, min_val_occurrence=0.0, early_stopping_threshold=0.0,
                                    w=None, alignment_weight=0.0, min_alert_match_similarity=0.0, bag_limit=2000):
            ...
            for group_b in groups:
                ...
                if group_b in sim_matrix[group_a]:
                    ...
                    if len(group_a.alerts) * len(group_b.alerts) > bag_limit:
                        s = similarity.get_group_similarity(group_a, group_b,
                                                            early_stopping_threshold=early_stopping_threshold, w=w,
                                                            min_alert_match_similarity=min_alert_match_similarity,
                                                            alignment_weight=alignment_weight, strategy='bag')
                    else:
                        s = similarity.get_group_similarity(group_a, group_b,
                                                            early_stopping_threshold=early_stopping_threshold, w=w,
                                                            min_alert_match_similarity=min_alert_match_similarity,
                                                            alignment_weight=alignment_weight, strategy='best')
        ...
        merged_group = merge.merge_group(merge_groups, min_alert_match_similarity=min_alert_match_similarity,
                                         max_val_limit=max_val_limit, min_key_occurrence=min_key_occurrence,
                                         min_val_occurrence=min_val_occurrence)
        ...
        print('Merge ' + lab[:-1] + ' with distance ' + str(1 - merge_similarities[merged_group]) + ' (' + str(
            1 - max_similarity) + ')\n')
        # Recursion stop criteria
        if len(groups) > 1:
            code = self.hierarchical_clustering_rec(groups, sim_matrix, merge_similarities, labels,
                                                    max_val_limit=max_val_limit, min_key_occurrence=min_key_occurrence,
                                                    min_val_occurrence=min_val_occurrence,
                                                    early_stopping_threshold=early_stopping_threshold, w=w,
                                                    min_alert_match_similarity=min_alert_match_similarity,
            ...
            code = 'node' + labels[merged_group].replace('+', '').replace('-', '') + ' <- Node$new("' + re.sub('[0-9]',
                                                                                                               '',
                                                                                                               labels[
                                                                                                                   merged_group]) + '", myPlotHeight=' + str(
                1 - merge_similarities[merged_group]) + ')\n'
        ...
        for merge_group in merge_groups:
            ...
            code += 'node' + labels[merge_group].replace('+', '').replace('-', '') + ' <- node' + labels[
                merged_group].replace('+', '').replace('-', '') + '$AddChild("' + re.sub('[0-9]', '', labels[
                merge_group]) + '", myPlotHeight=' + str(1 - merge_sim) + ')\n'
        return code
