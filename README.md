...
Quét alpha (trọng số LCS trong tổ hợp LCS+Jaccard) .
  - alpha = 0 ⇒ chỉ Jaccard
  - alpha = 1 ⇒ chỉ LCS
Đầu ra: data/out/comparison/alpha_sweep.{csv,md}
Cách chạy:
  python evaluation/compare_alpha.py                       # alpha mặc định [0, 0.25, 0.5, 0.75, 1]
  python evaluation/compare_alpha.py --alphas 0 0.5 1      # tuỳ chỉnh
  python evaluation/compare_alpha.py --threshold 0.4


Cách chạy phần số liệu per host:
  python evaluation/compare_per_host.py                 # delta=0.5, 1 lần chạy/host
  python evaluation/compare_per_host.py --delta 5       # đổi delta
  python evaluation/compare_per_host.py --repeats 3     # lấy trung bình 3 lần
  python evaluation/compare_per_host.py --quick         # chỉ chạy cup để test
Sinh bảng so sánh Pair-F1 theo TỪNG HOST (cup, spiral, onion, insect)
cho 4 cấu hình:
  - Baseline (Greedy)             : version=orig, pair_strategy=best
  - LCS+Jaccard                   : version=new,  pair_strategy=best, alpha=0.5
  - Sinkhorn OT                   : version=orig, pair_strategy=ot,   epsilon=0.01
  - Combined (LCS+Jaccard + OT)   : version=new,  pair_strategy=ot,   alpha=0.5, epsilon=0.01

Đầu ra: data/out/comparison/per_host_pairf1.{csv,md}


python evaluation/compare_min_length.py
