...
with open('data/out/cross_validation/sim_list.txt', 'w') as sim_out, open('data/out/cross_validation/cross_corr.txt', 'w') as cross_corr, open('data/out/cross_validation/reductions.txt', 'w') as reduct:
  ...
  cross_corr.write('threshold,delta,attack,tp,fp,fn,tn,tpr,fpr,prec,f1\n')
  ...
  for threshold in thresholds:
    ...
    for test_data in range(len(files)):
      print('Now testing files #' + str(test_data) + ' with threshold ' + str(threshold))
        ...
        for delta, groups in delta_dicts.items():
          for group in groups:
            ...
            if train_only_same_labels is True:
              force_label = group.attacks # Supervised - only allow meta alert generation for alerts with same label
      ...
      for file_group_index, delta_dicts in groups_dict.items():
        ...
        for delta, groups in delta_dicts.items():
          ...
          for group in groups:
            label.label_group(group)
            meta_alert, similarity = mam.get_most_similar_meta_alert(group, delta, threshold, min_alert_match_similarity=min_alert_match_similarity_val, max_val_limit=max_val_limit, min_key_occurrence=min_key_occurrence, min_val_occurrence=min_val_occurrence, bag_limit=bag_limit, w=w, alignment_weight=alignment_weight, partial=partial)
            meta_attack_labels = []
    ...
    for delta in deltas:
      ...
      for l in labels:
        ...
        cross_corr.write(str(threshold) + ',' + str(delta) + ',' + str(l) + ',' + str(tp[delta][l]) + ',' + str(fp[delta][l]) + ',' + str(fn[delta][l]) + ',' + str(tn[delta][l]) + ',' + str(tpr) + ',' + str(fpr) + ',' + str(prec) + ',' + str(f1) + '\n')

    # rows = actual, columns = prediction
    with open('data/out/cross_validation/attack_similarities_matrix_' + str(threshold).replace('.', '_') + '.txt', 'w') as similarities_out:
      similarities_str = ''
    ...
    with open('data/out/cross_validation/confusion_matrix_' + str(threshold).replace('.', '_') + '.txt', 'w') as confusion_out:
      confusion_str = ''
