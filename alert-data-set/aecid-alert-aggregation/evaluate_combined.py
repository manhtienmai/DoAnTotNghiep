"""
evaluate_combined.py — So sanh 4 cau hinh ket hop 2 cai tien
=============================================================
  Config 1: Baseline          (string_sim OFF, adjacent gating OFF)
  Config 2: String Sim only   (string_sim ON,  adjacent gating OFF)
  Config 3: Adj Gating only   (string_sim OFF, adjacent gating ON, theta=0.4)
  Config 4: Both combined     (string_sim ON,  adjacent gating ON, theta=0.4)

Dung cung tham so voi evaluate_aitads.py / aggregate_config.py
"""
...
from similarity import similarity as sim_pkg
...
# =====================================================================
configs = [
    # (name, string_sim_on, adj_gating_on)
    ('Baseline',           False, False),
    ('StringSim_only',     True,  False),
    ('AdjGating_only',     False, True),
    ('Both_combined',      True,  True),
]

# =====================================================================
# CACHE cho Baseline (khong can chay lai moi lan)
# =====================================================================
CACHE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         'data', 'out', 'evaluation')
CACHE_FILE = os.path.join(CACHE_DIR, 'combined_baseline_cache.pkl')
...
# HAM CHAY 1 CONFIG
# =====================================================================
def run_config(name, string_sim_on, adj_gating_on, use_cache=True):
    print(f"\n{'=' * 60}")
    print(f"  Config: {name}")
    print(f"  string_sim={'ON' if string_sim_on else 'OFF'}, "
          f"adj_gating={'ON (theta=' + str(THETA_ADJ) + ')' if adj_gating_on else 'OFF'}")
    print(f"{'=' * 60}")
    ...
    t_start = time.time()
    if adj_gating_on:
        groups_dict = read_input_improved.read_input(
            files, deltas, input_type, 0.0,
            aggregate_config.group_strategy, aggregate_config.group_type,
            theta_adj=THETA_ADJ, sim_func=sim_pkg.get_json_similarity
        )
    else:
        groups_dict = read_input_original.read_input(
            files, deltas, input_type
        )
...
def main():
    print("=" * 70)
    print("  EVALUATE COMBINED — 4 cau hinh ket hop 2 cai tien")
    ...
    print(f"  Adj gating: theta={THETA_ADJ}")
    print("=" * 70)

    use_cache = '--no-cache' not in sys.argv

    all_results = {}
    for name, str_sim, adj_gate in configs:
        eval_res, n_meta, n_groups, rt = run_config(name, str_sim, adj_gate, use_cache)
        all_results[name] = {
            'eval': eval_res,
            'n_meta': n_meta,
            'n_groups': n_groups,
        ...
        for delta in deltas:
            ...
            for l in labels:
                ...
                if bv['tp'] + bv['fp'] + bv['fn'] == 0:
                    # Kiem tra xem config khac co data khong
                    any_active = any(
